// This is vendored from https://github.com/benjamincharity/rehype-semantic-images
...
export interface RehypeSemanticElement {
  className?: string;
  disable?: boolean;
  id?: string;
}

export type RehypeSemanticElementPermanent = Omit<
  RehypeSemanticElement,
  "disable"
>;
...
export const rehypeSemanticImage: Plugin<
  [Partial<RehypeSemanticImages>?],
  Root
> = (options = {}) => {
  ...
  return (tree) => {
    visit(tree, { tagName: "img" }, (node, index, parent) => {
      ...
      if (
        !options_.elements.figure.disable &&
        !options_.elements.figcaption.disable &&
        typeof alt === "string"
      ) {
        const figcaption = h(
          "figcaption",
          { ...options_.elements.figcaption },
          alt
        );
      ...
      if (options_.elements.figure.disable) {
        (parent.children as HastElement[]).splice(index, 1, ...figureChildren);
      } else {
        const figure = h(
          "figure",
          { ...options_.elements.figure },
          ...figureChildren
        );
        (parent.children as HastElement[]).splice(index, 1, figure);
      }
    });
  };
};
...
function mergeOptions(
  userOptions: Partial<RehypeSemanticImages> = {}
): RehypeSemanticImagesRequired {
  return {
    ...defaultOptions,
    ...userOptions,
    elements: {
